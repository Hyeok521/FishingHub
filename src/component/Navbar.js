            ...
            <div className="side-menu" style={{width: width}}>
                ...
                <div className="side-menu-list" id="menu-list">
                    {menuList.map((menu, index) => (
                        (menu === '마이페이지' && auth?.userType === ROLE_ADMIN) ? <button key={index}>회원 관리</button> :
                        ...
                        <li key={index} className="Highdrop">
                            <Dropdown>
                                <Dropdown.Toggle variant="light" id={`dropdown-${menu}`}>
                                    {menu === '마이페이지' && auth?.userType === ROLE_ADMIN ? '회원관리' : menu}
                                </Dropdown.Toggle>
                                <Dropdown.Menu>
                                    {menu === "게시판" && (
                                        <>
                                    ...
                                    {menu === "마이페이지" && (auth?.userType === ROLE_ADMIN ?
                                        <Dropdown.Item eventKey="1" onClick={MemberList}>
                                            회원관리
                                        </Dropdown.Item> : (
