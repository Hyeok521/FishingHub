...
const Board = ({ boardMasterId }) => {
  ...
  const fetchPosts = () => {
    axios
      .get(process.env.REACT_APP_SERVER_URL + "/board/list", {
        params: { page: nowPage - 1, boardMasterId },
      })
      .then((response) => {
  ...
  const fetchBoardMaster = () => {
    axios
      .get(process.env.REACT_APP_SERVER_URL + "/board/master", {
        params: { boardMasterId },
      })
      .then((res) => {
  ...
  const handleDeletePost = (postId) => {
    ...
    axios
      .delete(process.env.REACT_APP_SERVER_URL + "/board/delete", {
  ...
  const handleCommentSave = () => {
    ...
    axios
      .post(
        process.env.REACT_APP_SERVER_URL + "/board/comment/write",
        {
      ...
      .then(() => {
        ...
        axios
          .get(process.env.REACT_APP_SERVER_URL + "/board/view", {
  ...
  const checkRole = (checkType) => {
    ...
    if (
      role === ROLE_ALL &&
      (checkTypeRole === ROLE_USER || checkTypeRole === ROLE_ADMIN)
    ) {
      return false;
    }
  ...
  return (
    <Container className="BoardBody">
      ...
      <BoardList
        ...
        boardMasterInfo={boardMasterInfo}
        checkRole={checkRole}
      />
      {/* 게시물 리스트 E */}
