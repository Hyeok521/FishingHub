...
const SighUp = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.userPw !== formData.reUserPw) {
    ...
    if (!userNickRegex.test(formData.userNickname)) {
      alert("닉네임은 2~10자로 제한됩니다.");
      return;
    }

    if (!userNmRegex.test(formData.userName)) {
    ...
    try {
      const response = await axios.post(
        process.env.REACT_APP_SERVER_URL + "/member/sign-up",
        formData
      );
